...
def fetch_events():
    ...
    while True:
        ...
        response = requests.get(GITHUB_EVENTS_API_URL, params=params, headers=headers)
...
def save_to_db(db_session, events):
    ...
    query = insert(Event).values(events).on_conflict_do_nothing(index_elements=['github_id'])
